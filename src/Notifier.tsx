...
// Import icon configuration from separate file
...
export const Notifier = ({
  ...
  title,
  message,
  icon = { library: "MaterialIcons", name: "visibility" },
  iconColor = "#007AFF",
  primaryButton,
  secondaryButton,
  ...
  const iconPulseRef = useRef<Animated.CompositeAnimation | null>(null);
  const iconBackgroundPulseRef = useRef<Animated.CompositeAnimation | null>(
    null
  );
  ...
  const resolveColor = useCallback(
    (color: ColorValue | undefined, fallback: string): string => {
  ...
  const getIconConfig = useCallback((): IconConfig => {
    ...
    return {
      ...
      backgroundColor:
        icon.backgroundColor || iconBackGroundColor || "transparent",
  ...
  const startPulsingAnimations = useCallback(() => {
    ...
    const iconBackgroundPulseAnimation = Animated.loop(
      Animated.sequence([
  ...
  const renderButtons = () => {
    ...
    if (secondaryButton) {
      const secondaryBorderColor = resolveColor(
        secondaryButton.secondaryButtonColor,
        defaultColors[currentTheme].secondaryButtonBorder
      );
      const secondaryTextColor = resolveColor(
        secondaryButton.secondaryButtonText,
        defaultColors[currentTheme].secondaryButtonText
      );
    ...
    if (primaryButton) {
      const primaryBgColor = resolveColor(
        primaryButton.primaryButtonColor,
        defaultColors[currentTheme].secondaryButtonBorder
      );
      const primaryTextColor = resolveColor(
        primaryButton.primaryButtonText,
        defaultColors[currentTheme].buttonText
      );
  ...
  const resolvedBackdropColor = resolveColor(
    backdropColor,
    defaultColors[currentTheme].backdropColor
  );

  const resolvedIconBackgroundColor = resolveColor(
